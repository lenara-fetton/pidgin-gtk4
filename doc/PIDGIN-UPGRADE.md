...
- **`pidginbackfill.[ch]`**
  ...
  - Line parsers: `_parse_html_line()`, `_parse_txt_line()` (dating with a `PidginLogClock`: `pidgin_log_clock_new(file_start, file_end)`, `_resume()`, `_get_last()`, `_free()`) and `_parse_file_name()`.
...
**Backfill dates, 2026-09-25:** the backfill added a day whenever an undated stamp was more than an hour before the previous line. libdiscord's logs interleave message lines stamped with the sender's time (history replays, and for a while UTC written as local time) with system lines ("X reacted with :kekw:", "┌──@user: …" reply headers) stamped with Pidgin 2's clock, so each system line looked like a new day: one 2021 channel log of 16,155 lines was dated into 2029, and the rows sorted to the bottom of that conversation. Also, `(12/31/1969 04:00:00 PM)` lines (a logged zero `time_t`) became 1969 rows, and a colourless `<span>…</span><b> text:</b>` system line whose text ends in a colon was read as a sender. Now a `PidginLogClock` dates the lines (`pidginbackfill.h` has the rules): only lines with a sender move the cursor and roll the day over, lines without one take the day nearest the cursor (until the first message line, so logs of joins and parts still roll over); no undated line is dated more than a day (message lines) or an hour (the others) after the file's mtime; an epoch stamp is dated at the cursor. Resuming inside a file restarts the cursor from its rows with a sender. The log viewer and the history plugin use the same clock (with the file's mtime). **Repair:** before its first run on an index (kv `backfill-date-repair-1`, account `""`), the backfill calls `pidgin_message_index_repair_misdated()`: log files that still exist with a row more than a day after their mtime, or at the epoch, lose their `indexed_files` entry and every row that holds only the log line (no stanza/origin/server/occupant id, no correction or reply, no reactions or receipts), and are indexed again in the same run. It scans on a private read-only connection and deletes in 500-row transactions. On a copy of the real `messages.db` (7.18M rows): 355 Discord/Steam files, 400,214 rows, 65 s including the reindex (in the backfill's worker thread); afterwards no epoch or future rows, the same row count, and rows with a Discord attachment snowflake over a day off went from 4,378 to 2,740 of 84,191 (the rest are replays of days before the file started and reposted attachments, which an undated stamp can't express). The 2021-10-14 log alone now spans 2021-10-14 07:08 to 2021-10-19 03:28, every snowflake row exactly libdiscord's 7 h skew off.
...
**Insert Image by HTTP upload (XMPP), 2026-09-23:**
- A conversation takes inserted images as uploads when its prpl's IPC `http-upload-available` says TRUE (the jabber prpl once the server's XEP-0363 service is found; see M8 *Landed (HTTP upload)*), it can send the conversation a file (IM `send_file`, room `chat_send_file`), and it doesn't take real inline images (`OPT_PROTO_IM_IMAGE` IMs, i.e. Discord, keep that path). Helpers: `pidgin_format_account_uploads_images()` and `pidgin_format_account_upload_max_size()` in `pidginmarkup.c`; `upload_images()` in `gtkconv.c`. The entry's caps then get `PIDGIN_FORMAT_IMAGE` (in rooms too), Insert Image is enabled, and a paste or drop of an image goes into the entry with no confirmation. The jabber prpl sends `PURPLE_CONV_UPDATE_FEATURES` when it finds the service, so open conversations follow.
- On send: the text with the `<IMG ID>`s removed (raw in HTML, escaped in XEP-0393 text) goes as one message if anything is left (as a reply when replying); then each image, in the entry's order (`pidgin_compose_entry_get_image_ids()`), is written as stored in the imgstore to `<profile>/pidgin4/paste/` (an image from a file is that file's bytes under its name; a paste was encoded by the paste prefs when pasted) and `serv_send_file`/`serv_chat_send_file`'d. The prpl uploads it and posts the URL; the file is removed when its transfer ends (the paste cleanup). An image above `http-upload-max-size` refuses the whole send with an error line, and the entry keeps everything. Corrections don't take this path.
- Without the service, or on file-only prpls, nothing changes: Insert Image stays off and a paste/drop gets the confirmation dialog.
- Live: `scripts/tests/xmpp-live/drive.sh` inserts an image in the room with Conversation → Insert Image, types text and sends: the text goes as a groupchat message, the image uploads to Prosody's `http_file_share`, bob receives its URL, and `pidgin4/paste/` is empty afterwards (37 checks pass).
- Conv selftest `test_upload_images`: caps and Insert Image with and without the service, text + Insert Image + paste → one `send_im` with the text and two `send_file`s in order under `pidgin4/paste/` (removed when the transfers end), image-only send, a dropped file, the size limit (error line, nothing sent, entry kept), a room (`chat_send_file`), and the confirm dialog without the service. The selftest prpl has togglable `http-upload-*` IPC, a call log and all started transfers.

**Inline media player hardening, 2026-09-23:** GTK 4.22's GStreamer backend `g_file_read()`s a `GtkMediaFile`'s GFile in its source-setup and fails `gtk_gst_media_file_source_setup_cb: assertion 'stream != NULL'` when that fails (a URL with no gvfs backend, a missing file). The media card's `GtkVideo` now gets a `GtkMediaFile` for an input stream `pidginattachment.c` opened asynchronously itself; if the open fails, or the stream reports an error later (not media, no codec), the player is removed (from an idle) and only the card stays. The conv selftest detects the backend at run time: the cards of the fake `.mp4`/`.ogg` files and URLs are checked with `/pidgin4/media/inline_playback` off, and `test_media_player` checks the player with the bundled `silence.wav` (kept), a non-media file, a missing file and a URL (removed, no critical). `PIDGIN4_SELFTEST_MEDIA_BACKEND=1` requires a backend; `=0` forces the no-backend path (`pidgin_media_backend_disable_for_tests()`); unset, the test follows what GTK has. Verified with GStreamer present under the script's `G_DEBUG=fatal-criticals`, both unset and `=1`, and with `=0`.

**Inline media hang, 2026-09-25:** the running pidgin4 froze for good. The main thread was in `g_thread_join()` inside GstPlay's dispose: `update_attachment()` removed a recycled row's media card, the `GtkVideo`'s `GtkGstMediaFile` went, and GstPlay waited for its thread. That thread was setting the pipeline to NULL and blocked in `gst_pad_stop_task()` on the source pad's stream lock, held by the `qtdemux` task stuck in `g_input_stream_read()` on a gvfs daemon stream: `media_player_new()` opened remote attachments with `g_file_new_for_uri()`, so `gvfsd-http` served them, and its HTTP connection had gone while the read waited, polling only the data socket (no cancellable fd), forever. 3 of the 12 players alive had stalled that way. Fixed in `pidginattachment.c` and `pidginimageloader.c`:
- The inline player never reads from the network. `pidgin_image_loader_fetch_async()` (new, additive) fetches a body raw through the loader's machinery (libsoup with timeouts and working cancellation, the allowlist, redirects only to allowed URIs, aesgcm decryption, a size cap, coalescing) and caches plain https bodies on disk under a key of their own (`sha256("raw\n" + uri)`, so an image load never finds and deletes one); a hit is `mmap`ped. The player plays a `GMemoryInputStream` of it; a local file still gets a stream opened asynchronously.
- The player is made on demand: the card shows a "Play Here" poster (`pidgin-media-poster`); pressing it allows that one URI in the loader, fetches (at most `PIDGIN_ATTACHMENT_MAX_FILE_SIZE`, 64 MiB; no poster for a remote file known to be larger) with a spinner, then puts a `GtkVideo` that plays at once in its place. A card going away cancels the fetch. On failure (not allowed, too large, HTTP, not media, no codec) a note `pidgin-media-error` ("This can't be played here.", the error as tooltip) takes the place of the poster or player. No pipelines (about 20 threads and a GL context each) for clips nobody plays, and no automatic downloads of every clip scrolled past.
- Tests: `test-imageloader` `/imageloader/fetch` (raw bytes, disk cache apart from images, caps, allowlist, redirects) and `/imageloader/fetch-cancel` (a stalled download ends within a second of the cancel); conv selftest `test_media_player` presses the poster (WAV plays; fake MP4, missing file and a non-media URL end in the note), no poster for a too-large URL, and a card whose download stalls goes at once. Meson tests 19 OK + `msgview-selftest` on Xvfb; `PIDGIN4_CONV_SELFTEST` PASS (389 checks) with `PIDGIN4_SELFTEST_MEDIA_BACKEND=1` and PASS (378) with `=0`.
...
- **Plugin IPC for UIs** (`jabber_http_upload_ipc_init()`, called from `jabber_plugin_init`; `purple_plugin_ipc_call(prpl_jabber, name, &ok, account)`):
  - `gboolean http-upload-available(PurpleAccount *account)`: TRUE when the account is connected, discovery found an upload service, and the `http_upload` account setting isn't FALSE.
  - `guint64 http-upload-max-size(PurpleAccount *account)`: the service's XEP-0128 `max-file-size`, or 0 when unknown (no service, or no stated limit). It is marshalled with `purple_marshal_POINTER__POINTER`, so `purple_plugin_ipc_call()` returns it as a pointer: read it with `GPOINTER_TO_SIZE()`. That's exact on 64-bit (the only target); a 32-bit build would truncate above 4 GiB.
  - When discovery finds the service (and from `jabber_http_upload_set_service()` with a JID), every open conversation of the account gets `purple_conversation_update(conv, PURPLE_CONV_UPDATE_FEATURES)` so the UI can re-evaluate what it offers. pidgin4 uses this for Insert Image by upload (M4).
  - Tested in `scripts/tests/jabber-m8/test_m8.c` (`test_http_upload_ipc`: no service, sign-on disco finding the service with a limit, the update, the `http_upload` switch, not connected, no stated limit).
...
**Landed (server features round 2), 2026-09-23.** XMPP only, all in `libpurple/protocols/jabber/` (new `blocking.[ch]` and `sfs.[ch]`; `jabber.c`, `presence.c`, `message.c`, `mam.c`, `httpupload.c`, `disco.c`, `libxmpp.c`). No libpurple API change; the ABI gate passes. Each feature is probed on the server and falls back to the previous behaviour when absent. **A follow-up UI agent implements pidgin4's side against this list** (no `pidgin4/` file was touched); the GTK 2 UI works unchanged apart from the new "Invisible" status and account option noted below.

- **IPC** (on the prpl-jabber plugin; same conventions as the message-semantics commands: the gboolean is the *return value* of `purple_plugin_ipc_call()`, `ok` only says it was dispatched; every command returns FALSE/NULL for a NULL, non-XMPP or unconnected account):
  - `GList *privacy-modes(PurpleAccount *account)` (`purple_marshal_POINTER__POINTER`): the privacy modes the **server** can enforce, as newly allocated strings (`g_list_free_full(l, g_free)`): `"allow-all"` always, plus `"deny-users"` when the server has XEP-0191. The names map to `PURPLE_PRIVACY_ALLOW_ALL`/`_DENY_USERS`; `"deny-all"`, `"allow-users"`, `"allow-buddylist"` never appear. The UI greys out the modes not listed (libpurple still filters incoming IMs locally in every mode, but presence and other stanzas get through).
  - `gboolean status-invisible-supported(PurpleAccount *account)`: the server lists `urn:xmpp:invisible:0`. The UI should hide or grey out the "invisible" status when FALSE (choosing it then is the same as available).
  - `gboolean report-spam-supported(PurpleAccount *account)`: the server lists both `urn:xmpp:blocking` and `urn:xmpp:reporting:1`.
  - `gboolean report-spam(PurpleAccount *account, const char *jid, const char *reason, gboolean abuse)` (`purple_marshal_BOOLEAN__POINTER_POINTER_POINTER_UINT`, pass `abuse` as a `guint`): blocks `jid` (normalized to the bare JID) with `<block><item jid=''><report xmlns='urn:xmpp:reporting:1' reason='urn:xmpp:reporting:spam'|'…:abuse'><text>reason</text></report></item></block>`; `reason` may be NULL (no `<text/>`). Without XEP-0377 it sends a plain XEP-0191 block and still returns TRUE; without XEP-0191 it returns FALSE and sends nothing. The server's block push then adds the JID to the deny list, as for any block. The report is only a block: unblocking later is `purple_privacy_deny_remove()`.
- **Blocking (XEP-0191).** Stock 2.14 already had it, and it stays as it was: the blocklist is fetched on connect and mirrored into the account's deny list (`purple_privacy_deny_add(…, local_only = TRUE)`, replacing it), pushes are applied the same way, and the prpl's `add_deny`/`rem_deny` send `<block/>`/`<unblock/>`. So the UI blocks and unblocks with the ordinary `purple_privacy_deny_add/remove(account, jid, FALSE)` (or `purple_privacy_deny()`), and `account->deny` is the server's list. New: `set_permit_deny` is implemented; it sends nothing (allow-all and deny-users need nothing, the other modes are logged as not server-enforced). **There is no XEP-0016 (privacy lists) code in this tree** (none in stock 2.14 either), so the "0016 fallback" of the brief doesn't exist: without XEP-0191, `add_deny` shows "Server doesn't support blocking" as before.
- **Invisible (XEP-0186).** A new status type `"invisible"` (`PURPLE_STATUS_INVISIBLE`, attribute `priority`, default 1) on every XMPP account, so the global "Invisible" saved status now applies to XMPP (it left XMPP accounts unchanged before). With `urn:xmpp:invisible:0` on the server: entering it sends `<invisible xmlns='urn:xmpp:invisible:0' probe='true'/>` and holds our presence back until the result (then sends it; the server doesn't broadcast it); leaving it sends `<visible/>` and a forced presence broadcast. If the server refuses `<invisible/>`, no presence is sent until the status changes (a notify error says so), so a refusal never shows an invisible user as online. Without XEP-0186 the status is sent as a plain available presence. Rooms already joined still get our directed presence.
- **MAM preferences (XEP-0313 §prefs).** After the account disco finds `urn:xmpp:mam:2`: `<prefs/>` is fetched; a default other than `always` is set to `always` (keeping the `<always/>`/`<never/>` lists). **kv key** `mam/prefs-set` = `1` is stored once the server has `always` (already, or after our set succeeded); while it is set no prefs IQ is sent at all. A failed set stores nothing (retried next login). **Account setting** `mam_prefs_always` (bool, default TRUE, shown in the account editor as "Ask the server to archive all messages (XEP-0313)"): FALSE leaves the server's preferences alone. GTK 2 has no kv store, so it fetches the prefs on each login (and sets them only while needed).
- **Stateless file sharing (XEP-0447 + 0446, 0264, 0300; XEP-0385 SIMS on receipt).**
  - Receiving: the first `<file-sharing xmlns='urn:xmpp:sfs:0'>` (with `<file xmlns='urn:xmpp:file:metadata:0'>`), or a legacy `<reference xmlns='urn:xmpp:reference:0' type='data'><media-sharing xmlns='urn:xmpp:sims:1'>`, adds these **meta keys** to `receiving-message-meta` (each only when known): `sfs-url` (the first http(s) `<url-data target=''/>` source; SIMS: the `<reference uri=''/>`), `sfs-name` (base name only), `sfs-size` (bytes), `sfs-media-type`, `sfs-width`, `sfs-height`, `sfs-desc`, `sfs-hash` (`algo:base64`; sha-256 preferred, then sha-512, sha3-256, sha3-512, blake2b), `sfs-thumbnail` (a `data:` URI: `data:` thumbnails as given, `cid:` resolved from a `<data xmlns='urn:xmpp:bob'/>` in the same message or the BoB cache; an unresolved `cid:` URI is passed as is; http thumbnails and URIs over 128 KiB are dropped), `sfs-disposition` (`inline`/`attachment`), `sfs-sims` (`1` for the SIMS form). The body is kept (normally the URL, plus the OOB fold as before), so old UIs and the log still show the link. Encrypted sources (`urn:xmpp:esfs:0`), jinglepub, attach-to (sources in a later message) and more than one file per message are not handled. The prpl logs `jabber: sfs: <name> (<type>, <size> bytes, <hash>) <url>`.
  - Sending: an HTTP upload computes the file's size, SHA-256 and, for PNG, JPEG, GIF and WebP, the width/height from the header (a small parser; no gdk-pixbuf in libpurple) in a `GTask` thread while the slot is requested and the file is PUT. The URL message then carries, next to the body and the OOB `<x/>`: `<file-sharing xmlns='urn:xmpp:sfs:0' [disposition='inline' for images]><file xmlns='urn:xmpp:file:metadata:0'><media-type/><name/><size/>[<width/><height/>]<hash xmlns='urn:xmpp:hashes:2' algo='sha-256'/></file><sources><url-data xmlns='http://jabber.org/protocol/url-data' target='GET URL'/></sources></file-sharing>`. No thumbnail or `<desc/>` is sent (no IPC to supply them yet). JPEG EXIF rotation isn't applied to the dimensions.
  - `urn:xmpp:sfs:0` is advertised.
- **Explicit Message Encryption (XEP-0380).** An `<encryption xmlns='urn:xmpp:eme:0' namespace=''/>` is reported unless the message was decrypted: the OMEMO plugin removes `<encrypted/>` (and `<encryption/>`) when it decrypts, so an OMEMO (`eu.siacs.conversations.axolotl`) message counts as decrypted when no `<encrypted/>` is left; every other namespace counts as not decrypted. **Meta keys** `eme-namespace` and `eme-name` (the `name` attribute, else a known name: OMEMO for `eu.siacs.conversations.axolotl`/`urn:xmpp:omemo:1`/`:2`, "OpenPGP for XMPP" for `urn:xmpp:openpgp:0`, "Legacy OpenPGP" for `jabber:x:encrypted`, "OTR" for `urn:xmpp:otr:0`, else the namespace). The body is untouched for a message-meta UI, which should render "encrypted with <eme-name>" itself when `eme-namespace` is set. **Without message-meta** (GTK 2), a body that is the usual boilerplate (the Conversations/Dino/Gajim/this OMEMO plugin texts, `?OTR…`, or anything up to 300 characters that mentions "encrypt" and the scheme's name) is replaced by "[Encrypted with <name>, which this client does not support]"; other bodies are kept. Nothing OMEMO-specific is added to the meta: the OMEMO UI uses `omemo-list-devices`. `urn:xmpp:eme:0` is advertised.
- **Last user interaction in presence (XEP-0319).** Incoming `<idle xmlns='urn:xmpp:idle:1' since=''/>` sets the resource's idle start (absolute: it wins over XEP-0256 `seconds` and gets no `<delay/>` correction; a future `since` counts as now), so `purple_prpl_got_user_idle()` and the buddy list's idle display work for modern clients; presence without it (and without XEP-0256) is not idle. Our own presence carries `<idle since=''/>` next to the XEP-0256 query while the account is idle (`purple_presence_set_idle()`, e.g. from pidgin4's idle module), rooms included. `urn:xmpp:idle:1` is advertised. No UI work needed.
- **Tests:**
  - `scripts/tests/jabber-m8/run.sh`: 573 checks (405 before), all passing. It now also links libsoup-3.0. New cases: blocklist fetch and pushes (incl. a refused push from a stranger), block/unblock through the privacy API, privacy modes and `set_permit_deny`, `report-spam` with and without XEP-0377 and XEP-0191, `<invisible/>`/`<visible/>` with the held-back presence, a refusal and the no-0186 fallback, the MAM prefs round trip and kv gate (incl. an error and the opt-out), SFS parsing (hash choice, BoB/data/unresolved/http thumbnails, SIMS), PNG/GIF/JPEG/WebP headers, local file metadata, the `<file-sharing>` element sent after an HTTP upload to a local SoupServer, EME meta and the no-meta fallback, and XEP-0319 parse/emit.
  - `scripts/tests/xmpp-live/drive.sh` against Prosody 13.0.6: 62 checks, all passing. The server now runs with `default_archive_policy = "roster"`, `bounce_blocked_messages`, a small test module advertising XEP-0377 and logging reports, and alice/bob subscribed to each other. A test-only plugin (`scripts/tests/xmpp-live/ctl-plugin.c`, loaded into each instance via the scratch profile) runs commands from `$work/ctl-NAME`. Verified live: MAM prefs `roster` → `always` on both accounts, and no prefs IQ after bob's restart (the kv key in `messages.db`); XEP-0191 block, push, alice's message bounced and never shown to bob, unblock, push, messages flow again; `privacy-modes` and `report-spam` over IPC with the report arriving at the server; XEP-0319 idle sent, parsed and shown in bob's buddy list as 600 s, then cleared; `<file-sharing>` with SHA-256 on the UI upload and with 40×30 dimensions on a PNG upload, both parsed by bob; the invisible status falling back to available.
  - **Unit-tested only:** XEP-0186 (Prosody 13 has no invisibility module, and none is among the fetched community modules); received thumbnails and SIMS; EME (needs a second client that encrypts with something else).
  - The GTK 2 `pidgin -n` from the prefix ran 20 s under Xvfb on a scratch profile and loaded and unloaded `libxmpp.so` cleanly.
- **For the UI agent (pidgin4):**
  - Blocking: a "Block"/"Unblock" item (buddy menu, conversation menu) = `purple_privacy_deny_add/remove(account, bare_jid, FALSE)`; "Report spam…" (with a reason entry and an "abuse" choice) = `report-spam`, offered when `report-spam-supported`, otherwise just "Block". The privacy dialog greys out the modes missing from `privacy-modes`.
  - Invisible: offer the `invisible` status only when `status-invisible-supported` is TRUE for the account (re-query on `signed-on`; it is FALSE while offline).
  - Files: render a file card from the `sfs-*` keys (name, size, type, thumbnail from `sfs-thumbnail`, dimensions to reserve space before an image loads), linking `sfs-url` and verifying a download against `sfs-hash` when one is made; keep treating the body URL as the fallback.
  - Encryption: when `eme-namespace` is set and the text is the sender's fallback, show "Encrypted with <eme-name>, which this client does not support" instead.
  - Account editor: `mam_prefs_always` (bool, default TRUE), label as above.
- **Open issues:**
  - No XEP-0016: ALLOW_USERS / ALLOW_BUDDYLIST / DENY_ALL aren't server-enforced (presence and IQs from anyone still arrive).
  - The upload's `<file-sharing>` has no thumbnail or description yet; an IPC to pass them (or a thumbnail made by the UI) would be the next step. An OMEMO aesgcm upload would need `urn:xmpp:esfs:0` sources, which aren't built.
  - SFS: only the first file of a message is reported; `attach-to` source messages aren't joined.
  - Invisible: the status is offered even on servers without XEP-0186 (status types can't depend on the server); after a stream-management resume the prpl doesn't know whether the old session was invisible, so switching to visible while disconnected may leave the server session invisible until the next status change.
  - The URL message of an upload still bypasses `jabber_message_send()` (no receipt request, markable or `sending-message-meta`), as noted under HTTP upload.
...
- **IPC**, registered on the plugin in `load` with the M8 signatures (the debug log says `Registered IPC commands send-correction, send-reaction, send-retraction and send-reply`). All return FALSE unless the account is a connected Discord account on a message-meta UI.
  ...
  Actions on thread messages go to the thread's channel. The results come back through the gateway like changes from any other client. The second round adds `send-marker` and `mam-fetch-older` (below).
- Threads render as before.

**Discord, second round** (2026-09-23; five plugin commits, each gated on `native_meta` like the rest, and each passing the gates below first):
- **Stickers, GIFs and link embeds inline** (`d459f53`).
  - Stickers (`sticker_items`) become `<img src="https://media.discordapp.net/stickers/ID.png?size=160" alt="name">` (`.gif` for GIF stickers, format 4). Lottie stickers (format 3) have no static image in the API, so they are shown as `[name]`.
  - Tenor/Giphy messages (`gifv` embeds) stay the bare URL, plus an `<img>` of the GIF itself: the embed's `.gif` image, else Tenor's MP4 URL with its `AAAPo` format code turned into `AAAAC` (the GIF), or Giphy's `giphy.mp4` → `giphy.gif`, else a `.gif` thumbnail. Otherwise only the thumbnail is shown.
  - Other embeds become a compact block instead of the coloured-border one: `<b><a href=url>title</a></b>` (or the author's name), the description cut at 200 characters, a rich embed's fields (`<b>name</b> value`), and `<img src>` of the image or the thumbnail. Discord's proxied copy (`proxy_url`, `images-ext-1/2.discordapp.net`) is preferred, so the site itself isn't contacted.
  - New meta keys, taken from the first embed for a future card: `embed-type`, `embed-title`, `embed-description` (all of it), `embed-url` and `embed-image` (the image the block shows).
  - An embed-only `MESSAGE_UPDATE` that adds something to show is now a `message-corrected` with the whole body (text plus the embed block). A `receiving-message-meta` goes just before it, with `correction-of`, the new key **`embed-only-update` = `1`** and the `embed-*` keys, and no ids. The UI may apply that correction silently. pidgin4 doesn't read the key yet, so it shows these as edited, and the table is dropped on the next idle. If the UI doesn't show the message, nothing is written.
  - pidgin4's image loader allowlist (`pidginimageloader.c`, this branch) now also has `images-ext-1.discordapp.net`, `images-ext-2.discordapp.net`, `media.tenor.com`, `c.tenor.com`, `media.giphy.com` and `i.giphy.com`. Stickers are on `media.discordapp.net`, which was already listed.
- **Read state both ways** (`251a2fe`).
  - Messages from others carry `markable` = `1`, so pidgin4 calls `send-marker` for the newest one shown.
  - New IPC **`send-marker(account, conv_name, message_id, marker)`** (M8 signature) → `POST /channels/{id}/messages/{id}/ack` with the ack token, on the message's own channel for thread messages. `marker` may be `displayed`, NULL or `""`, or `acknowledged`; `received` returns FALSE. A per-connection map of the newest read message per channel sends each read once.
  - With message-meta it **replaces** the `conversation-updated` ack: that handler now returns at once, so a read isn't acked twice. With `/pidgin4/conversations/send_markers` off, pidgin4 acks nothing.
  - Gateway **`MESSAGE_ACK`** (read on another client) → `message-receipt(account, conv, message_id, "displayed", <our account's username>)`. The username is what pidgin4 compares to decide "read elsewhere", which clears the unread state. Reads it already knew (the echo of our own ack) and thread acks are skipped.
- **Scroll-back** (`bd0cec6`).
  - New IPC **`mam-fetch-older(account, conv_name, before_id, count)`** (M8 signature) → `GET /channels/{id}/messages?before={before_id}&limit={count}`, for guild channels, DMs and group DMs. `count` is clamped to 1–100, and a NULL or `""` `before_id` means the newest page.
  - The page is shown oldest first through the normal path, as `PURPLE_MESSAGE_DELAYED` with `mam` = `1` and `mam-query` = `older`.
  - Then the plugin emits **`mam-query-done(account, conv_name, first_id, last_id, complete)`** on its own handle. `first_id` is the oldest message (the next `before_id`) and `last_id` the newest; `complete` (a `guint`) means the page was shorter than asked. A failed request ends with no ids and FALSE.
  - The signal is registered in `load` only when the ui_info has `message-meta` (the log says `Registered signal mam-query-done`), and unregistered in `unload`. Stock Pidgin doesn't have it.
- **Reactions on history messages** (`10256a1`).
  - Instead of the "reacted with" text lines, who reacted is fetched with `GET …/messages/{id}/reactions/{emoji}?limit=100`, and each user becomes a `message-reaction(…, add=TRUE)`, with the same emoji and sender forms as live reactions. Our own ones also update the own-reactions cache.
  - The fetches are lazy and polite:
    - one request at a time, 350 ms apart (under 3/s);
    - after a response whose `X-RateLimit-Remaining` is 0, the next waits for its `X-RateLimit-Reset-After` (429s were already retried);
    - only the newest N messages queued; N is the new account setting **`reaction_history_limit`** (default 25; 0 turns it off), an account option shown only on message-meta UIs;
    - messages the UI discarded (already shown) are skipped;
    - the queue and its timer go with the connection.
  - A message with more than 100 reactors on one emoji gets the first 100.
- **Friends' games as status attributes** (`0a35382`). On a message-meta UI the online, idle, dnd, offline and mobile status types get `game` and `game_app_id` besides `message` (`purple_status_type_new_with_attrs`, 2.14 API). Friends' presences set them through `purple_prpl_got_user_status()`: `PRESENCE_UPDATE` without a guild (renames included) and READY's presences.
  - The value is the first activity of type 0 (playing), 1 (streaming), 2 (listening), 3 (watching) or 5 (competing), or the legacy `game` object: its `name` and `application_id`.
  - Custom statuses (type 4) aren't games. Without a game, or offline, both are unset.
  - pidgin4's `presence_game()` already shows `game` on the row and in the tooltip. It adds "Playing X" unless the status text already names the game. `message` is unchanged.
- **Gates** (before every one of the five commits):
  - Symbol gate: prints nothing, and `scripts/check-abi.sh --no-abidiff` passes. New libpurple imports, all 2.14: `purple_signal_register`/`_unregister`, `purple_marshal_VOID__POINTER_POINTER_POINTER_POINTER_UINT`, `purple_marshal_BOOLEAN__POINTER_POINTER_POINTER_UINT`, `purple_account_get_status_type` and `purple_status_type_get_attr`.
  - Stock Pidgin 2.14.14 and pidgin4 (`-m -n -d`) each ran 20 s under Xvfb + `dbus-run-session`, on a scratch copy of `~/.purple` (without logs or `messages.db`) whose `plugins/libdiscord.so` points at the new build. Both exited 0 on SIGTERM, unloaded Discord, and printed no criticals (pidgin4 ran with `G_DEBUG=fatal-criticals`). `accounts.xml` still has 34 accounts and is identical once sorted and with `active=` ignored.
  - pidgin4's log has the new IPC list and `Registered signal mam-query-done`. Stock Pidgin's log has the IPC list only.
  - `scripts/tests/discord-m9/run.sh` (281 checks; also passes with `--asan`) now covers every feature above with sample payloads in the API documentation's shapes. It can capture REST requests and answer them itself (`capture_requests`/`respond()`).
  - The **stock-output gate** is new: `stock_dump.c` runs 26 payloads on a UI without message-meta (every sample, `MESSAGE_ACK`, the unseen-update ack, a history page with reactions, presences) and prints the writes, signals, REST requests, buddy statuses, status types, account options and the plugin's signals. It is built against the branch and against `c5f7c41`, and the two outputs must be identical, which they are.
  - No account was signed in.
...
- Deletions of messages not shown in this process fall back to the text line.
- **pidgin4 doesn't hear Discord's `mam-query-done` yet.** `pidginconvmeta.c`'s `connect_mam_signal()` only connects to `prpl-jabber`, so a Discord scroll-back page is prepended only when the 30 s timeout fires. That file was out of scope for this round. The fix is small: connect to the signal on any loaded prpl that registers it, e.g. also `prpl-eionrobb-discord`, in `connect_mam_signal()` and `plugin_load_cb()`, keeping one handler per plugin.
- **Embed cards (follow-up).** The embed block is plain HTML: bold linked title, text and an image. A nicer card would read the `embed-*` keys (`embed-type`, `-title`, `-description`, `-url`, `-image`) from the message's meta in `pidginconvmeta.c`. It would draw a bordered box under the message: title as a link, two or three lines of description, the thumbnail at the side for `article`/`link` and full width for `image`/`gifv`, and the provider's name. The plugin would then stop appending the HTML block when the UI says it draws cards (a new ui_info key). With `embed-only-update` = `1`, the UI should also apply the correction without the "edited" mark, and not log a correction line.
- History reactions: only the newest `reaction_history_limit` messages get their reactors, and a message's older reactions beyond that show nothing. After a restart, the reactors of a message shown before (and discarded as a duplicate) aren't fetched again.
- A `MESSAGE_ACK` for a thread is skipped, because pidgin4 shows threads inside the parent channel's conversation. So reading a thread on another client doesn't clear anything.
- With message-meta the unseen-update ack is gone. A conversation whose newest incoming message isn't markable (for example one filled only from the index) isn't acked until a new message arrives.
- Tenor GIF URLs are derived from the MP4 URL's format code, a Tenor convention that isn't documented. If Tenor changes it, the thumbnail is shown instead.
