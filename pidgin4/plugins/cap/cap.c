...
/*
 * pidgin4 (M7): ported from pidgin/plugins/cap. The statistics database is
 * <profile>/pidgin4/cap.db (Pidgin 2's cap keeps <profile>/cap.db; both
 * UIs can run on one profile without sharing a SQLite file), the prefs
 * frame is a GtkGrid, and "now()", which SQLite doesn't have (those
 * inserts always failed), is datetime('now').
 */
#include "cap.h"
...
static void signed_on(PurpleConnection *gc) {
	...
	sql = sqlite3_mprintf("insert into cap_my_usage values(%Q, %Q, %d, datetime('now'));", account_id, protocol_id, 1);
...
static void signed_off(PurpleConnection *gc) {
	...
	sql = sqlite3_mprintf("insert into cap_my_usage values(%Q, %Q, %d, datetime('now'));", account_id, protocol_id, 0);
	sqlite3_exec(_db, sql, NULL, NULL, NULL);
	sqlite3_free(sql);
...
static gboolean create_database_connection() {
	...
	/* build the path: <profile>/pidgin4/cap.db (profile contract, rule 5:
	 * Pidgin 2's cap keeps <profile>/cap.db; the first time, pidgin4
	 * starts from a copy of it so the statistics carry over) */
	path = g_build_filename(pidgin_user_dir(), "cap.db", (gchar *)NULL);
	if (!g_file_test(path, G_FILE_TEST_EXISTS)) {
		gchar *old = g_build_filename(purple_user_dir(), "cap.db", (gchar *)NULL);

		if (g_file_test(old, G_FILE_TEST_IS_REGULAR)) {
			GFile *src = g_file_new_for_path(old);
			GFile *dst = g_file_new_for_path(path);
			GError *error = NULL;

			if (!g_file_copy(src, dst, G_FILE_COPY_NONE, NULL, NULL, NULL, &error)) {
				purple_debug_warning("cap", "Could not copy %s: %s\n", old, error->message);
				g_error_free(error);
			} else {
				purple_debug_info("cap", "Copied %s to %s\n", old, path);
			}
			g_object_unref(src);
			g_object_unref(dst);
		}
...
static void insert_status_change_from_purple_status(CapStatistics *statistics, PurpleStatus *status) {
	...
	purple_debug_info("cap", "Executing: insert into cap_status (buddy, account, protocol, status, event_time) values(%s, %s, %s, %s, datetime('now'));\n", buddy_name, account_id, protocol_id, status_id);

	sql = sqlite3_mprintf("insert into cap_status values (%Q, %Q, %Q, %Q, datetime('now'));", buddy_name, account_id, protocol_id, status_id);
...
static void insert_word_count(const char *sender, const char *receiver, guint count) {
	/* TODO! */
	/* dbi_result result; */
	/* result = dbi_conn_queryf(_conn, "insert into cap_message values(\'%s\', \'%s\', %d, datetime('now'));", sender, receiver, count); */
}
...
static gboolean plugin_unload(PurplePlugin *plugin) {
	...
	g_clear_pointer(&_my_offline_times, g_hash_table_destroy);
	_signals_connected = FALSE;
...
static GtkWidget *
spin_row(GtkGrid *grid, int row, const char *text, const char *pref)
{
	...
	gtk_label_set_xalign(GTK_LABEL(minutes), 0);
	gtk_widget_set_hexpand(spin, TRUE);
	gtk_label_set_mnemonic_widget(GTK_LABEL(label), spin);
	if (purple_prefs_exists(pref))
		gtk_spin_button_set_value(GTK_SPIN_BUTTON(spin), purple_prefs_get_int(pref));
	gtk_grid_attach(grid, label, 0, row, 1, 1);
	gtk_grid_attach(grid, spin, 1, row, 1, 1);
	gtk_grid_attach(grid, minutes, 2, row, 1, 1);
	g_signal_connect(spin, "value-changed", G_CALLBACK(numeric_spinner_prefs_cb), (gpointer)pref);
	return spin;
}
...
static CapPrefsUI * create_cap_prefs_ui() {
	...
	/* pidgin4: a GtkGrid instead of the GtkTable */
	ui->ret = gtk_box_new(GTK_ORIENTATION_VERTICAL, 18);
	gtk_widget_set_margin_top(ui->ret, 10);
	gtk_widget_set_margin_bottom(ui->ret, 10);
	gtk_widget_set_margin_start(ui->ret, 10);
	gtk_widget_set_margin_end(ui->ret, 10);
	ui->cap_vbox = pidgin_make_frame(ui->ret, _("Statistics Configuration"));

	ui->table_layout = gtk_grid_new();
	grid = GTK_GRID(ui->table_layout);
	gtk_grid_set_row_spacing(grid, 6);
	gtk_grid_set_column_spacing(grid, 12);
	ui->threshold_input = spin_row(grid, 0, _("Threshold:"), "/plugins/gtk/cap/threshold");
	ui->msg_difference_input = spin_row(grid, 1, _("Maximum response timeout:"),
		"/plugins/gtk/cap/max_msg_difference");
	ui->last_seen_input = spin_row(grid, 2, _("Maximum last-seen difference:"),
		"/plugins/gtk/cap/max_seen_difference");
	gtk_box_append(GTK_BOX(ui->cap_vbox), ui->table_layout);

	g_signal_connect(ui->ret, "destroy", G_CALLBACK(cap_prefs_ui_destroy_cb), ui);

	return ui;
}

static void cap_prefs_ui_destroy_cb(GtkWidget *widget, gpointer user_data) {
	CapPrefsUI *ui = user_data;
	if(_db && _plugin_pointer != NULL && purple_plugin_is_loaded(_plugin_pointer)) {
