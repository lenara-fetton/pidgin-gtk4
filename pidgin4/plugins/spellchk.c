...
/*
 * pidgin4 (M7): the autocorrect of Pidgin 2's spellchk on the
 * PidginComposeEntry's buffer, unchanged. What changed: the word list is a
 * GListStore of SpellchkWord objects instead of a GtkListStore; the last
 * word is checked in the entry's "pre-send" signal (Pidgin 2 stopped its
 * "message_send"), which holds the message back once when it replaced
 * something, as before; the editor is a GtkColumnView. The word list is
 * still <profile>/dict, in Pidgin 2's format and shared with it.
 * (Spelling itself is libspelling in the compose entry, not this plugin.)
 */
...
/* One correction: a GObject so the list can be a GListStore. */
#define SPELLCHK_TYPE_WORD (spellchk_word_get_type())
G_DECLARE_FINAL_TYPE(SpellchkWord, spellchk_word, SPELLCHK, WORD, GObject)

struct _SpellchkWord
{
	GObject parent;
	char *bad;
	char *good;
...
G_DEFINE_FINAL_TYPE(SpellchkWord, spellchk_word, G_TYPE_OBJECT)

static void
spellchk_word_finalize(GObject *obj)
{
	SpellchkWord *w = SPELLCHK_WORD(obj);

	g_free(w->bad);
	g_free(w->good);
	G_OBJECT_CLASS(spellchk_word_parent_class)->finalize(obj);
}

static void
spellchk_word_class_init(SpellchkWordClass *klass)
{
	G_OBJECT_CLASS(klass)->finalize = spellchk_word_finalize;
}

static void
spellchk_word_init(SpellchkWord *w)
{
}

static SpellchkWord *
spellchk_word_new(const char *bad, const char *good, gboolean word_only,
                  gboolean case_sensitive)
{
	SpellchkWord *w = g_object_new(SPELLCHK_TYPE_WORD, NULL);

	w->bad = g_strdup(bad);
	w->good = g_strdup(good);
	w->word_only = word_only;
	w->case_sensitive = case_sensitive;
	return w;
}

static int
spellchk_word_compare(gconstpointer a, gconstpointer b, gpointer data)
{
	return g_utf8_collate(SPELLCHK_WORD((gpointer)a)->bad, SPELLCHK_WORD((gpointer)b)->bad);
}

#define N_WORDS() g_list_model_get_n_items(G_LIST_MODEL(model))

/* The item at @i (the store keeps its reference). */
static SpellchkWord *
word_at(GListStore *store, guint i)
{
	SpellchkWord *w = g_list_model_get_item(G_LIST_MODEL(store), i);
...
substitute_simple_buffer(GtkTextBuffer *buffer)
{
	...
	for (i = 0; text != NULL && i < N_WORDS(); i++) {
		SpellchkWord *w = word_at(model, i);
		gchar *cursor;
		glong char_pos;
		...
		if ((cursor = g_strrstr(text, w->bad)))
		{
			char_pos = g_utf8_pointer_to_offset(text, cursor);
			gtk_text_buffer_get_iter_at_offset(buffer, &start, char_pos);
			gtk_text_buffer_get_iter_at_offset(buffer, &end, char_pos + g_utf8_strlen(w->bad, -1));
			gtk_text_buffer_delete(buffer, &start, &end);

			gtk_text_buffer_get_iter_at_offset(buffer, &start, char_pos);
			gtk_text_buffer_insert(buffer, &start, w->good, -1);
...
substitute_word(gchar *word)
{
	...
	for (i = 0; i < N_WORDS(); i++) {
		SpellchkWord *w = word_at(model, i);
		gboolean case_sensitive = w->case_sensitive;
		const char *bad = w->bad;
		gchar *tmpbad = NULL;
...
spellchk_free(spellchk *spell)
{
	...
	if (spell->view != NULL) {
		...
		g_signal_handlers_disconnect_matched(buffer,
		                                     ...
		                                     0, 0, NULL, NULL,
		                                     spell);
		/* pidgin4: the marks are named; drop them so a later attach
		 * (the plugin loaded again) can create them again */
		if (!gtk_text_mark_get_deleted(spell->mark_insert_start))
			gtk_text_buffer_delete_mark(buffer, spell->mark_insert_start);
		if (!gtk_text_mark_get_deleted(spell->mark_insert_end))
			gtk_text_buffer_delete_mark(buffer, spell->mark_insert_end);
		g_object_remove_weak_pointer(G_OBJECT(spell->view), (gpointer *)&spell->view);
	}
	g_free(spell->word);
	g_free(spell);
}
...
static gboolean
pre_send_cb(PidginComposeEntry *entry, spellchk *spell)
{
	GtkTextBuffer *buffer;
	GtkTextIter start, end;
	GtkTextMark *mark;
	...
	if (replaced)
	{
		/* hold the message back once, so the correction is seen */
		spell->ignore_correction_on_send = TRUE;
		return TRUE;
	}
	return FALSE;
...
spellchk_new_attach(PurpleConversation *conv)
{
	...
	view = GTK_TEXT_VIEW(pidgin_conv_get_compose_entry(gtkconv));
	...
	g_signal_connect(view, "pre-send", G_CALLBACK(pre_send_cb), spell);
	return;
}
...
static void load_conf(void)
{
	...
	model = g_list_store_new(SPELLCHK_TYPE_WORD);
		...
		if (*buf != '#') {
			...
			else if (!g_ascii_strncasecmp(buf, "GOOD ", 5))
			{
				...
				if (*bad && *good && g_hash_table_lookup(hashes, bad) == NULL) {
					...
					w = spellchk_word_new(bad, good, complete, case_sensitive);
					g_list_store_append(model, w);
					g_object_unref(w);
				}
				bad[0] = '\0';
				complete = TRUE;
	...
	g_list_store_sort(model, spellchk_word_compare, NULL);
}

/**************************************************************************
 * The editor (a GtkColumnView over the GListStore)
 **************************************************************************/
...
static GtkMultiSelection *selection;

static void save_list(void);

#define ITEM_KEY "spellchk-item"
#define FIELD_KEY "spellchk-field"
...
static void
text_editing_cb(GtkEditableLabel *label, GParamSpec *pspec, gpointer data)
{
	SpellchkWord *w = g_object_get_data(G_OBJECT(label), ITEM_KEY);
	int field = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(label), FIELD_KEY));
	const char *text;
	char **target;

	if (w == NULL || gtk_editable_label_get_editing(label))
		return;
	text = gtk_editable_get_text(GTK_EDITABLE(label));
	target = field == FIELD_BAD ? &w->bad : &w->good;
	if (text == NULL || *text == '\0') {
		/* no empty corrections: put the old text back */
		gtk_editable_set_text(GTK_EDITABLE(label), *target);
		gtk_widget_error_bell(GTK_WIDGET(label));
		return;
	}
	if (!purple_strequal(text, *target)) {
		g_free(*target);
...
static void
check_toggled_cb(GtkCheckButton *button, gpointer data)
{
	SpellchkWord *w = g_object_get_data(G_OBJECT(button), ITEM_KEY);
	int field = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(button), FIELD_KEY));
	gboolean active = gtk_check_button_get_active(button);
	guint pos;

	if (w == NULL)
		return;
	if (field == FIELD_WORD_ONLY) {
		if (w->word_only == active)
			return;
		w->word_only = active;
		/* as Pidgin 2: partial-word replacements are always exact case */
		w->case_sensitive = !active;
	} else {
		if (w->case_sensitive == active)
			return;
		w->case_sensitive = active;
	}
	/* rebind the row so the other column follows */
	if (g_list_store_find(model, w, &pos))
		g_list_model_items_changed(G_LIST_MODEL(model), pos, 1, 1);
	save_list();
}

static void
setup_text_cb(GtkSignalListItemFactory *factory, GtkListItem *item, gpointer field)
{
	GtkWidget *label = gtk_editable_label_new("");

	g_object_set_data(G_OBJECT(label), FIELD_KEY, field);
	g_signal_connect(label, "notify::editing", G_CALLBACK(text_editing_cb), NULL);
	gtk_list_item_set_child(item, label);
}

static void
bind_text_cb(GtkSignalListItemFactory *factory, GtkListItem *item, gpointer field)
{
	GtkWidget *label = gtk_list_item_get_child(item);
	SpellchkWord *w = gtk_list_item_get_item(item);

	g_object_set_data(G_OBJECT(label), ITEM_KEY, NULL);
	gtk_editable_set_text(GTK_EDITABLE(label),
	                      GPOINTER_TO_INT(field) == FIELD_BAD ? w->bad : w->good);
	g_object_set_data(G_OBJECT(label), ITEM_KEY, w);
}

static void
setup_check_cb(GtkSignalListItemFactory *factory, GtkListItem *item, gpointer field)
{
	GtkWidget *button = gtk_check_button_new();

	gtk_widget_set_halign(button, GTK_ALIGN_CENTER);
	g_object_set_data(G_OBJECT(button), FIELD_KEY, field);
	g_signal_connect(button, "toggled", G_CALLBACK(check_toggled_cb), NULL);
	gtk_list_item_set_child(item, button);
}

static void
bind_check_cb(GtkSignalListItemFactory *factory, GtkListItem *item, gpointer field)
{
	GtkWidget *button = gtk_list_item_get_child(item);
	SpellchkWord *w = gtk_list_item_get_item(item);
	gboolean word_only = GPOINTER_TO_INT(field) == FIELD_WORD_ONLY;

	g_object_set_data(G_OBJECT(button), ITEM_KEY, NULL);
	gtk_check_button_set_active(GTK_CHECK_BUTTON(button),
	                            word_only ? w->word_only : w->case_sensitive);
	/* case matching only means something for whole words */
	gtk_widget_set_sensitive(button, word_only || w->word_only);
	g_object_set_data(G_OBJECT(button), ITEM_KEY, w);
}

static void
unbind_cb(GtkSignalListItemFactory *factory, GtkListItem *item, gpointer field)
{
	g_object_set_data(G_OBJECT(gtk_list_item_get_child(item)), ITEM_KEY, NULL);
}

static GtkColumnViewColumn *
make_column(const char *title, gboolean text, int field)
{
	GtkListItemFactory *factory = gtk_signal_list_item_factory_new();
	GtkColumnViewColumn *column;

	g_signal_connect(factory, "setup", G_CALLBACK(text ? setup_text_cb : setup_check_cb),
	                 GINT_TO_POINTER(field));
	g_signal_connect(factory, "bind", G_CALLBACK(text ? bind_text_cb : bind_check_cb),
	                 GINT_TO_POINTER(field));
	g_signal_connect(factory, "unbind", G_CALLBACK(unbind_cb), GINT_TO_POINTER(field));
...
static void list_add_new(void)
{
	...
	gboolean case_sensitive = gtk_check_button_get_active(GTK_CHECK_BUTTON(case_toggle));
	char *tmpword = g_utf8_casefold(word, -1);
	SpellchkWord *w;
	guint i;

	for (i = 0; i < N_WORDS(); i++) {
		SpellchkWord *other = word_at(model, i);
		gboolean match;

		if (case_sensitive && other->case_sensitive)
		{
			/* If they're both case-sensitive, then compare directly.
			 * Otherwise, they overlap. */
			match = purple_strequal(other->bad, word);
		}
		else
		{
			char *bad = g_utf8_casefold(other->bad, -1);
		...
		if (match) {
			...
			purple_notify_error(NULL, _("Duplicate Correction"),
				...
				gtk_editable_get_text(GTK_EDITABLE(bad_entry)));
			return;
		}
	}
	g_free(tmpword);

	w = spellchk_word_new(word, gtk_editable_get_text(GTK_EDITABLE(good_entry)),
		gtk_check_button_get_active(GTK_CHECK_BUTTON(complete_toggle)), case_sensitive);
	g_list_store_insert_sorted(model, w, spellchk_word_compare, NULL);
	g_object_unref(w);

	gtk_editable_set_text(GTK_EDITABLE(bad_entry), "");
	gtk_editable_set_text(GTK_EDITABLE(good_entry), "");
	gtk_check_button_set_active(GTK_CHECK_BUTTON(complete_toggle), TRUE);
	gtk_check_button_set_active(GTK_CHECK_BUTTON(case_toggle), FALSE);
...
static void list_delete(void)
{
	...
	if (selection == NULL)
		return;
	selected = gtk_selection_model_get_selection(GTK_SELECTION_MODEL(selection));
	/* from the end, so the positions stay valid */
	for (i = N_WORDS(); i-- > 0; )
...
static void save_list()
{
	...
	for (i = 0; i < N_WORDS(); i++) {
		...
		g_string_append_printf(data, "COMPLETE %d\nCASE %d\nBAD %s\nGOOD %s\n\n",
		                       w->word_only, w->case_sensitive, w->bad, w->good);
	}
...
static void on_selection_changed(GtkSelectionModel *sel, guint position, guint n_items,
	gpointer data)
{
	GtkBitset *selected = gtk_selection_model_get_selection(sel);

	gtk_widget_set_sensitive((GtkWidget*)data, !gtk_bitset_is_empty(selected));
	gtk_bitset_unref(selected);
}
...
plugin_unload(PurplePlugin *plugin)
{
	...
	for (convs = purple_get_conversations(); convs != NULL; convs = convs->next)
	{
		...
		spell = g_object_get_data(G_OBJECT(entry), SPELLCHK_OBJECT_KEY);
		if (spell == NULL)
			continue;

		g_signal_handlers_disconnect_by_func(entry, pre_send_cb, spell);
		/* spellchk_free() disconnects from the buffer and drops the marks */
		g_object_set_data(G_OBJECT(entry), SPELLCHK_OBJECT_KEY, NULL);
	}
...
static void whole_words_button_toggled(GtkCheckButton *complete_toggle, GtkCheckButton *case_toggle)
{
	gboolean enabled = gtk_check_button_get_active(complete_toggle);

	gtk_check_button_set_active(case_toggle, !enabled);
	gtk_widget_set_sensitive(GTK_WIDGET(case_toggle), enabled);
}

static void
config_destroy_cb(GtkWidget *widget, gpointer data)
{
	g_clear_object(&selection);
	bad_entry = good_entry = complete_toggle = case_toggle = NULL;
}
...
get_config_frame(PurplePlugin *plugin)
{
	...
	g_signal_connect(ret, "destroy", G_CALLBACK(config_destroy_cb), NULL);

	vbox = pidgin_make_frame(ret, _("Text Replacements"));
	gtk_widget_set_vexpand(vbox, TRUE);

	g_clear_object(&selection);
	selection = gtk_multi_selection_new(G_LIST_MODEL(g_object_ref(model)));
	tree = gtk_column_view_new(GTK_SELECTION_MODEL(g_object_ref(selection)));
	gtk_column_view_set_show_row_separators(GTK_COLUMN_VIEW(tree), TRUE);
	gtk_column_view_append_column(GTK_COLUMN_VIEW(tree),
		make_column(_("You type"), TRUE, FIELD_BAD));
	gtk_column_view_append_column(GTK_COLUMN_VIEW(tree),
		make_column(_("You send"), TRUE, FIELD_GOOD));
	gtk_column_view_append_column(GTK_COLUMN_VIEW(tree),
		make_column(_("Whole words only"), FALSE, FIELD_WORD_ONLY));
	gtk_column_view_append_column(GTK_COLUMN_VIEW(tree),
		make_column(_("Case sensitive"), FALSE, FIELD_CASE));
	gtk_box_append(GTK_BOX(vbox),
		pidgin_make_scrollable(tree, GTK_POLICY_NEVER, GTK_POLICY_ALWAYS, -1, 200));

	hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, PIDGIN_HIG_BOX_SPACE);
	gtk_box_append(GTK_BOX(vbox), hbox);
	button = gtk_button_new_with_mnemonic(_("_Delete"));
	g_signal_connect(button, "clicked", G_CALLBACK(list_delete), NULL);
	gtk_box_append(GTK_BOX(hbox), button);
	gtk_widget_set_sensitive(button, FALSE);
	g_signal_connect(selection, "selection-changed", G_CALLBACK(on_selection_changed), button);
	...
	gtk_check_button_set_active(GTK_CHECK_BUTTON(complete_toggle), TRUE);
	g_signal_connect(complete_toggle, "toggled",
	                 G_CALLBACK(whole_words_button_toggled), case_toggle);
	gtk_box_append(GTK_BOX(vbox2), complete_toggle);

	gtk_check_button_set_active(GTK_CHECK_BUTTON(case_toggle), FALSE);
	gtk_box_append(GTK_BOX(vbox2), case_toggle);

	button = gtk_button_new_with_mnemonic(_("_Add"));
	gtk_widget_set_valign(button, GTK_ALIGN_END);
	g_signal_connect(button, "clicked", G_CALLBACK(list_add_new), NULL);
	gtk_box_append(GTK_BOX(hbox), button);
	g_signal_connect(bad_entry, "changed", G_CALLBACK(on_entry_changed), button);
	g_signal_connect(good_entry, "changed", G_CALLBACK(on_entry_changed), button);
	gtk_widget_set_sensitive(button, FALSE);
