/*
 * pidgin4: Release Notification (M7 port of pidgin/plugins/relnot.c).
 *
...
/*
 * Asks pidgin.im for the newest Pidgin 2 release at most once a day, on
 * sign-on, and shows a mini-dialog in the buddy list when there is one.
 * The request still goes through libpurple's purple_util_fetch_url
 * (plugins link nothing but GTK, GLib and libpurple), with the account
 * proxy settings; the dialog is a PidginMiniDialog. Like every plugin it
 * is off until loaded, and pidgin4 never loads it by itself. The pref
 * (/plugins/gtk/relnot/last_check) is shared with Pidgin 2.
 */
...
static void
release_show(PidginMiniDialog *dialog, GtkButton *button, gpointer data)
{
                 ...
                 const gchar *response, size_t len, const gchar *error_message)
{
	...
	PidginMiniDialog *release_dialog;
	GString *message;
	int i = 0;
	...
	release_dialog = pidgin_mini_dialog_new(_("New Version Available"), message->str,
	                                        "dialog-information");
	pidgin_mini_dialog_add_button(release_dialog, _("Later"), NULL, NULL);
	pidgin_mini_dialog_add_button(release_dialog, _("Download Now"), release_show, NULL);
	pidgin_blist_add_alert(GTK_WIDGET(release_dialog));
...
do_check(void)
{
	...
	if (!last_check || time(NULL) - last_check > MIN_CHECK_INTERVAL) {
		...
		url = g_strdup_printf("https://%s/version.php?version=%s&build=%s",
		                      host, purple_core_get_version(), "purple");
